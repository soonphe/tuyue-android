...
public class CustomStatusBar extends LinearLayout {
    private static final String TAG="CustomerStatusBar";
    private static final String NETWORK_STATE_CHANGE="android.net.conn.CONNECTIVITY_CHANGE";
    private Context mContext;
    View mStatusBarView;
    TextView mBattery,mBattery_state,mCurrentTime; //电量 电量状态 当前时间
    ...
    public CustomStatusBar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        initView(); //初始化view
        initWifiService(); //初始化wifi
        initBatteryService();//初始化电量
        initTimeService();//初始化系统时间
    }
    ...
    private void initView() {
        mStatusBarView= LayoutInflater.from(mContext).inflate(R.layout.status_bar, this);
        mBattery = mStatusBarView.findViewById(R.id.battery);
        mBattery_state = mStatusBarView.findViewById(R.id.battery_state);
        mCurrentTime = mStatusBarView.findViewById(R.id.currentTime);
        mBatteryIcon= mStatusBarView.findViewById(R.id.battery_icon);
        mWifiImage = mStatusBarView.findViewById(R.id.wifi);

        //设置当前时间
        String currentTimerStr=new SimpleDateFormat("HH:mm").format(new Date());
        mCurrentTime.setText(currentTimerStr);
    }
    ...
    class TimeChangeReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            mCurrentTime.setText(currentTimeStr);
        }
    }
    //自定义电量显示
    private class BatteryBroadcast extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            mBattery.setText(levelPercent + "%");
            //电池状态
            int status = extras.getInt(BatteryManager.EXTRA_STATUS/*电池状态*/);
            mBattery_state.setVisibility(GONE);
            switch (status) {
                case BatteryManager.BATTERY_STATUS_CHARGING://充电
                    Log.d(TAG, "充电中..");
                    mBattery_state.setVisibility(VISIBLE);
                    mBattery_state.setText("充电中");
                    break;
    ...
    private BroadcastReceiver wifiIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(NETWORK_STATE_CHANGE)) {
                boolean isNetOK = NetWorkUtils.hasNetWork(mContext);
                Log.d(TAG, "网络状态发生变化,是否可用：" + isNetOK);
                if (isNetOK) {
                    initWifiState();
                } else {
                    mWifiImage.setBackgroundResource(R.drawable.wifi0);
                }
    ...
    public void updateWifiStrength() {
        int strength = getStrength(mContext);
        if (strength >= 0 && strength <= 3)
            mWifiImage.setBackgroundResource(wifiStateImgs[strength]);
    ...
    public void unregisterBroadcast() {
        ...
        mContext.unregisterReceiver(wifiIntentReceiver);
